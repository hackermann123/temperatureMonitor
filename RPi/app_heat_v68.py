# Temperature Monitoring System - Flask Backend (v6.8 - FIXES RENAME + GRAPH DISPLAY NAMES)
...
class DataLogger:
    ...
    STORES BOTH SENSOR NAME AND ID IN HEADERS FOR GRAPH LOOKUP
    """
    ...
    def start_session(self, sensors):
        """Create new logging session file with sensor NAMES in headers AND store ID mapping"""
        with self.lock:
            ...
            try:
                ...
                # Store ID mapping: name -> sensor_id for graph lookups
                self.sensor_id_map = {s["name"]: sid for sid, s in sensors.items()}
                ...
                print(f"[LOGGER] Sensor ID Map: {self.sensor_id_map}")
                return filename
            except Exception as e:
            ...
            with open(filepath, 'r') as f:
                ...
                for line in lines[1:]:
                    ...
                    if len(values) > 1:
                        timestamp = values[0]
                        readings = {}
                        # Use friendly names from headers
...
def rename_probe():
    ...
    try:
        ...
        print(f"[API] Rename request: {data}")
        sensor_id = data.get("sensorid")
        name = data.get("name")
        print(f"[API] Extracted: sensorid={sensor_id}, name={name}")
        if not sensor_id or not name:
            print(f"[API] Missing parameter: sensorid={sensor_id}, name={name}")
...
def startup_sequence():
    ...
    print("[STARTUP] Initializing Temperature Monitoring System v6.8 - FIXES RENAME + GRAPH DISPLAY NAMES")
    ...
    print("[STARTUP] Probe management: Includes RENAME (fixed) and DELETE functionality!")
    print("[STARTUP] Parser: Accepts hex sensor IDs (8+ chars) with colon, space, or comma separator")
    print("[STARTUP] Graphs: Now display custom probe names instead of sensor IDs!")
