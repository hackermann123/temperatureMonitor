# Temperature Monitoring System - Flask Backend
# Author: Your Name
...
def start_logging():
    ...
    try:
        # Create logger with new folder
        logger_instance = DataLogger(folder)
        
        # Start session
        sensors = data_manager.get_sensors()
        filename = logger_instance.log_reading.__self__.start_session(sensors) if hasattr(logger_instance.log_reading.__self__, 'start_session') else logger_instance.start_session(sensors)
        
        # Start logging thread
        logging_thread = LoggingThread(data_manager, logger_instance, state_machine, duration, interval)
